...
describe("SessionStore LRU cap (evictOldest)", () => {
  ...
  it("spares an in-flight session even when it is the oldest, evicting the oldest idle instead", () => {
    // A session running a long heavy tool has the stalest lastSeen but is the
    // worst victim: closing it tears down its still-open response. evictOldest
    // must skip inFlight > 0 and pick the oldest IDLE entry.
    const store = new SessionStore(2);
    const a = fakeEntry(1_000); // oldest, but mid heavy-tool
    a.entry.inFlight = 1;
    const b = fakeEntry(2_000); // oldest idle -> the correct victim
    const c = fakeEntry(3_000);
    store.register("a", a.entry);
    store.register("b", b.entry);
  ...
  it("falls back to the oldest entry when every session is in-flight", () => {
    // The cap must always make room; if no session is idle, the oldest busy one
    // is still evicted rather than letting the store grow past the limit.
    const store = new SessionStore(2);
    const a = fakeEntry(1_000);
    a.entry.inFlight = 1;
    const b = fakeEntry(2_000);
